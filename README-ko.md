...
기본 포트는 `14522`입니다.
서버가 HTTPS를 사용하면 `UseSSL`을 `true`로, HTTP(로컬/LAN)이면 `false`로 설정하세요.

### HTTPS (선택 사항)

서버는 Kestrel을 통해 HTTPS를 지원합니다. 도메인과 SSL을 사용하려면:

1. [certbot](https://certbot.eff.org/)을 설치하고 인증서를 발급합니다:
```bash
...
openssl pkcs12 -export -out cert.pfx \
  -inkey /etc/letsencrypt/live/<도메인>/privkey.pem \
  -in /etc/letsencrypt/live/<도메인>/fullchain.pem \
  -password pass:<비밀번호>
```

3. `appsettings.json`을 설정합니다:
```json
{
  "Urls": "https://+:14522",
  "SslDirectory": "<cert.pfx가 있는 디렉터리 경로>",
  "SslPassword": "<비밀번호>"
}
```

4. 클라이언트의 `TideUpTogether.js`의 주소를 도메인으로 변경합니다.

> SSL 설정이 없으면 서버는 HTTP 모드로 동작합니다 (로컬/LAN 사용에 적합).

# 빌드

클라이언트(`TideUpTogether.js`)는 해당 게임의 기반인 RPG 만들기 MV 특성 상 번들링과 같은 과정을 거치지 않고 바로 사용합니다.

서버를 빌드하려면 .NET SDK(10 이상)가 필요합니다.
